...
export class BufferBigEndian {
  ...
  pushUint8(value: number): void {
    if (value > 255) {
      console.error("BufferBigEndian pushUint8 value need <= 255");
    }
    this.buffer.push(value);
  }

  getUint16(): number | null {
    if (this.readOffset + 2 > this.buffer.length) {
      return null;
    }
    let uint1 = this.getUint8();
    let uint2 = this.getUint8();
    if (uint1 === null || uint2 === null) {
      return null;
    }
  ...
  pushStringWithUtf8(value: string): number | null {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      if (ch1 < 128) {
        ...
        if (unicodeOk.ok) {
          if (unicodeOk.unicode === undefined) {
            return null;
          }
  ...
  getStringWithUtf8(len: number): string {
    ...
    while (read < len) {
      ...
      if (unicodeLen.unicode === undefined) {
        break;
      }
